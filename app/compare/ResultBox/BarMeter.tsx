...
export function BarMeter ({ startup, active, recovery, properties }: IBarMeterProps) {
  ...
  const data = [{
    ...
    ProjectileRecovery: projectileRecoveryVal,
      ...
      <ResponsiveContainer width="100%" height={60}>
        <BarChart data={data} key={uuid()} layout='vertical' margin={utils.barMargin}>
          <XAxis height={20} type="number" id="x" ticks={xTicks} tick={{ fill: '#ddd' }}
            ...
            padding={{ left: 1, right: 2 }} style={{ fontSize: '15px' }}
          />
          ...
          {isProjectile && (<>
            <Bar dataKey="Projectile" stackId="a" fill={projectileFill}>
            ...
            <Bar dataKey="ProjectileRecovery" stackId="a" fill={recoveryFill}>
              <LabelList dataKey="ProjectileRecovery" position="insideRight" fill="#ddd" fontSize="12" />
            </Bar>
          </>
          )}
          {!isProjectile && (
          ...
          {utils.ticksArray.map(tick => {
            return (<ReferenceLine key={uuid()} x={tick} stroke={utils.barColors.strokeColor} strokeWidth={2} isFront />);
          })}
          {Array.isArray(parsedActive) && parsedActive.map((tick, index) => {
            return (
              <ReferenceLine key={uuid()} x={tick - 1} stroke={utils.barColors.activeFill} strokeWidth={25} isFront>
